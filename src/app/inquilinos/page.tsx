...
export default function InquilinosPage() {
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        includeItbms: paymentForm.includeItbms,
      ...
      if (paymentForm.referenceNumber) {
        payload.referenceNumber = paymentForm.referenceNumber
      }
      if (paymentForm.paidAt) {
        payload.paidAt = paymentForm.paidAt
      }
      if (paymentForm.receiptImage) {
        payload.receiptImage = paymentForm.receiptImage
      }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedTenant || !editFormData.name || !editFormData.email) {
      toast({
        title: 'Error',
        description: 'Por favor completa todos los campos requeridos',
        variant: 'destructive',
    ...
    try {
      ...
      const response = await fetch(`/api/users/${selectedTenant.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        toast({
          ...
          description: 'El inquilino se ha actualizado exitosamente.',
        })
        ...
        toast({
          title: 'Error',
          description: data.error || 'No se pudo actualizar el inquilino',
          variant: 'destructive',
        })
      }
    } catch (error) {
  ...
  const handleDelete = async (tenant: Tenant) => {
    if (!confirm(`¿Estás seguro de eliminar al inquilino "${tenant.name}"?`)) {
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        toast({
          ...
          description: 'El inquilino se ha eliminado exitosamente.',
        })
        fetchData()
      } else {
        toast({
          title: 'Error',
          description: data.error || 'No se pudo eliminar el inquilino',
          variant: 'destructive',
        })
      }
    } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2 md:col-span-2">
                <div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg">
                  <Switch
                    id="includeItbms"
                    checked={paymentForm.includeItbms}
                    onCheckedChange={(checked) => setPaymentForm({...paymentForm, includeItbms: checked})}
                  />
                  <div className="flex-1">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between font-bold pt-2 border-t">
                    ...
                    <span className="text-primary">
                      ${paymentForm.includeItbms 
                        ? (parseFloat(paymentForm.amount) * 1.07).toFixed(2)
